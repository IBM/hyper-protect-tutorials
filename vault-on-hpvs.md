...
## Introduction
*IBM Vault* offers offers a centralized, secure way to manage secrets such as encryption keys, API tokens, and database credentials across the entire application lifecycle. While it's a leading product in its category, storing all critical secrets in one place also makes it a high-value target for attacks that could compromise an entire enterprise.

[IBM Hyper Protect Services](https://www.ibm.com/products/hyper-protect) uses [IBM Secure Execution (IBM SEL)](https://www.ibm.com/docs/en/linux-on-systems?topic=management-secure-execution) to protect the entire compute lifecycle. IBM SEL is a Linux on IBM Z and LinuxONE implementation of a [Trusted Execution Environments (TEE)](https://en.wikipedia.org/wiki/Trusted_execution_environment), to protect data while it's being used and when it’s loaded into memory and actively processed. 

Vault secures its secrets on disk using a Vault Master Key. This key can be further protected with a Key Encryption Key (KEK) from a Hardware Security Module (HSM), enabling Vault features such as [auto-unseal](https://www.vaultproject.io/docs/concepts/seal) and [seal wrapping](https://www.vaultproject.io/docs/enterprise/sealwrap).  
...
When Vault is run within Confidential Computing, the [Hyper Protect Virtual Server (HPVS)](https://www.ibm.com/docs/en/hpvs), which runs on the [Hyper Protect Container Runtime (HPCR)](https://www.ibm.com/docs/en/hpvs/2.2.x?topic=servers-downloading-hyper-protect-container-runtime-image), ensures that workloads cannot be inspected by a bad-actor or super-user with the ability to do a memory dump, as shown below:
...
Initial Root Token: [REMOVE DUE TO SECURITY SCANS]
...
- Login to vault with command `./vault login token=[REMOVE DUE TO SECURITY SCANS]`:
```
...
Key                  Value
---                  -----
token                [REMOVE DUE TO SECURITY SCANS]
